...
resources/
  App.rdef             app signature/version/icon resources (see Makefile)
  make_icon.py         generates the HVIF vector icon embedded in App.rdef
...
`rc`/`xres` embed `resources/App.rdef` (app name, signature, and the
tilted-newspaper vector icon) and `resattr` copies them to attributes,
which is where Tracker and Deskbar read the icon from. `make install` also
puts a link on the Desktop. The icon is HVIF generated by
`resources/make_icon.py`; re-run it and paste the hex into `App.rdef` to
change the design.
...
2. No pull-to-refresh / manual refresh button yet -- currently only
